var ui = {
	...
	// Render the Calendar
	"renderCalendar" : function(mm,yy,e){
		evnt = e;
		// HTML renderers
		var _html = "";
		var cls = "";
...
*/
		...
		for(var i=0;i>=last;i++){
			...
			for(var j=0;j<d.length;j++){
				...
				if(dow == 0){
					...
					_html += '<td class="' +cls+ '" id="'+id+'">' + dow.substr(-2) + '<br/><a class="content" href="/events/'+eid+'">'+msg+'</a></td>';
				}else{
		...
		$('#last').unbind('click').bind('click',function(){
			getEvents(prv.getMonth(),prv.getFullYear());
		});
		
		$('#current').unbind('click').bind('click',function(){
			getEvents(now.getMonth(),now.getFullYear());
		});
		
		$('#next').unbind('click').bind('click',function(){
			getEvents(nxt.getMonth(),nxt.getFullYear());
		});
...
getEvents = function(mm,yy) {
	...
	$.ajax( {
	    ...
	    ui.renderCalendar(mm,yy,{"event" : data});
	  }
	});
};
